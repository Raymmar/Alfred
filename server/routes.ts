...
- Preserve original text content exactly`,
    ...
    // New endpoint for transcription stage
    app.post("/api/projects/:id/transcribe", requireAuth, async (req: AuthRequest, res: Response) => {
      try {
        const projectId = parseInt(req.params.id);
        const [project] = await db.query.projects.findMany({
          where: and(
        ...
        // Get user settings for custom prompts
        const [user] = await db.query.users.findMany({
          where: eq(users.id, req.user!.id),
          limit: 1,
        });
    ...
    // New endpoint for analysis stage
    app.post("/api/projects/:id/analyze", requireAuth, async (req: AuthRequest, res: Response) => {
      try {
        const projectId = parseInt(req.params.id);
        const [project] = await db.query.projects.findMany({
          where: and(
        ...
        if (!project.transcription) {
          return res.status(400).json({ message: "No transcription found. Please transcribe the audio first." });
        }
...
${project.transcription}`;
        ...
        const summaryResponse = await openai.chat.completions.create({
          ...
          messages: [
            {
              role: "system",
              content: "You are a helpful assistant that summarizes text and extracts key insights.",
            },
            {
              role: "user",
        ...
        const tasksResponse = await openai.chat.completions.create({
          ...
          messages: [
            {
              role: "system",
              content: "You are a helpful assistant that identifies and extracts actionable tasks from text.",
            },
            {
              role: "user",
content: todoPrompt,
        ...
        // Create tasks if they're not empty/invalid
        const tasksText = tasksResponse.choices[0]?.message?.content || "";
        if (!isEmptyTaskResponse(tasksText)) {
    ...
    // Keep existing combined endpoint for backward compatibility
    app.post("/api/projects/:id/process", requireAuth, async (req: AuthRequest, res: Response) => {
      try {
        // First attempt transcription
        const transcriptionResponse = await fetch(`${req.protocol}://${req.get('host')}/api/projects/${req.params.id}/transcribe`, {
          method: 'POST',
          headers: {
        ...
        if (!transcriptionResponse.ok) {
          ...
          return res.status(transcriptionResponse.status).json(error);
        }
        ...
        if (!analysisResponse.ok) {
          ...
          return res.status(analysisResponse.status).json(error);
        }
