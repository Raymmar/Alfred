...
interface ChatOptions {
  ...
  isAutoGenerated?: boolean;  // New flag to differentiate auto-generated content
}
...
export async function createChatCompletion({
  ...
  context,
  promptType = 'system',
  isAutoGenerated = false  // Default to false for regular chat messages
}: ChatOptions) {
...
${projectContext.todos?.map(t => `- ${t.text} (${t.completed ? 'Completed' : 'Pending'})`).join('\n') || 'No tasks'}`;
  ...
  try {
    ...
    // Only store in chat messages if it's not auto-generated content
    if (!isAutoGenerated) {
      ...
      // Only create embeddings for actual chat messages
      await Promise.all([
        createEmbedding({
          contentType: 'chat',
